...
| `list_books` | `list_books() -> list[dict]` | Multi-book navigation read model. Returns only existing `book` columns `id`, `series_id`, `book_number`, `version`, and `position`, in deterministic series/position/id order; read-only and parameterized. Implemented with the same result shape by `SQLiteAdapter` and `InMemorySQLiteAdapter` (Plan M). |
...
| GET `/api/pipeline/books` | api_operations | M — list persisted books with owned project snapshots |
...
| BookProjects | M | id, series_id, book_number, version, position, projects: ProjectSnapshot[] |
